...
<script>
 var formDatos =$('#ajaxFormDatos')[0];
    ...
    // Quitar todos los contadores anteriores
    $('.error-count').remove();
    
    // Función simple para contar errores en un tab
    ...
    const camposPorTab = {
      ...
      $(document).ajaxComplete(function(event, xhr, settings) {
        if(xhr.status === 422) { // Si hay error de validación
            // Revisar cada tab
            Object.entries(camposPorTab).forEach(([tabId, campos]) => {
                let numErrores = contarErroresTab(campos);
                
                if(numErrores > 0) {
                    // Agregar el contador como subíndice
                    $(`a[href="#${tabId}"]`).append(
                        `<span class="error-count" style="color: red; font-size: 12px;"><sub>(${numErrores})</sub></span>`
                    );
                }
            });
        }
