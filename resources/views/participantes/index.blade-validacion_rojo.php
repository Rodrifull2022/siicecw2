 ...
 var formDatos =$('#ajaxFormDatos')[0];
            $('#saveBtn').click(function(){
                ...
                $('.error-messages').html('');
                 ...
                 // Remover clase de error de todos los tabs
    $('.nav-link').removeClass('text-danger');
    
    // Función para verificar errores y marcar el tab
    function marcarTabConError(campos, tabId) {
        for(let campo of campos) {
            if($('#' + campo + 'Error').text().trim() !== '') {
                $('a[href="#' + tabId + '"]').addClass('text-danger');
                return true;
            }
        }
        return false;
    ...
    const tabsCampos = {
        ...
        if(xhr.status === 422) { // Error de validación
            // Revisar cada tab
            for(let [tab, campos] of Object.entries(tabsCampos)) {
                marcarTabConError(campos, tab);
            }
        }
    });
